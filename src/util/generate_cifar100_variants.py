'''
Code to generate CIFAR-100-CS. Augmented from
...
def uniform_noise(x, severity=1):
    c = [0.07, 0.11, 0.15, 0.20, 0.30][severity - 1]
...
test_data = dset.CIFAR100(f'{ROOT}/datasets', train=False, download=True)
convert_img = trn.Compose([trn.ToTensor(), trn.ToPILImage()])

os.makedirs(PATHS[args.variant], exist_ok=True)
...
for method_name in d.keys():
    ...
    np.save(f'{PATHS[args.variant]}/' + d[method_name].__name__ + '.npy',
            np.array(cifar_c).astype(np.uint8))
    
    np.save(f'{PATHS[args.variant]}/labels.npy',
